...
def part1(instructions):
    register_history = execute_instructions(instructions)
    interesting_cycles = [20, 60, 100, 140, 180, 220]
    return sum(map(
        lambda x: x * register_history[x-1],
        interesting_cycles
    ))
...
def part2(instructions):
    ...
    for i in range(0, len(crt), width):
        out.append(''.join(crt[i:i+width]))
    return out

if __name__ == '__main__':
    instructions = []
    for line in get_input('2022/day10/input.txt'):
    ...
    print('part 2:')
    for line in part2(instructions):
        print(line)

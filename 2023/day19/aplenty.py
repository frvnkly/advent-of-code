        ...
        def workflow(part):
            for rule in rules:
                ...
                if "<" in condition:
                    ...
                    if (
                        part[category] is None or
                        part[category] >= int(val)
                    ):
                        continue
                elif ">" in condition:
                    category, val = condition.split(">")
                ...
                return self.workflows[result](part)
            
        return workflow

    def process(self, part):
        return self.workflows["in"](part)
...
def get_workflows_and_parts():
    ...
    for j in range(i + 1, len(lines)):
        part = { "x": None, "m": None, "a": None, "s": None }
        for segment in lines[j][1:-1].split(","):
            category, val = segment.split("=")
            part[category] = int(val)
        parts.append(part)

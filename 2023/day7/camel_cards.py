...
def get_hands_and_bids():
    hands = []
    bids = []
    for line in get_input("2023/day7/input.txt"):
        hand, bid = line.split()
        hands.append(hand)
        bids.append(int(bid))
...
def get_hand_type(hand):
    ...
    if combo_counts.get(2) == 2: return 2
    if combo_counts.get(3) == 1 and combo_counts.get(1) == 2: return 3
    if combo_counts.get(3) == 1 and combo_counts.get(2) == 1: return 4
...
def part1(hands, bids):
    ...
    for i in range(len(ranked_hands)):
        hand = ranked_hands[i]
        rank = i + 1
        total_winnings += hand_bid_lookup[hand] * rank
